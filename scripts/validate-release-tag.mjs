...
export function expectedReleaseTag(version) {
  if (typeof version !== 'string' || version.trim() === '') throw new Error('package version must be a non-empty string');
  return `v${version}`;
}

export function validateReleaseTag(tag, version) {
  const expected = expectedReleaseTag(version);
  if (tag !== expected) throw new Error(`Tag ${tag || '<missing>'} does not match package.json version ${version}; expected ${expected}`);
  return expected;
}
